...
/**
 * SMB Security & IT Hygiene Copilot – MVP (TypeScript, Next.js)
 * קובץ בודד שמוכן לרוץ ב-Vercel.
 */

// -------------------------------
// Affiliate Catalog (replace URLs)
// -------------------------------
type Vendor = { name: string; url: string; blurb: string; affiliate?: boolean };
const AFFILIATE: Record<string, Vendor[]> = {
  passwordManager: [
    {
  ...
  mfa: [
    ...
    { name: "Duo MFA", url: "https://duo.com/product/multi-factor-authentication-mfa", blurb: "Flexible MFA across apps." }
  ],
  mdm: [
    {
      name: "Microsoft Intune (MDM)",
      url: "https://www.microsoft.com/en-us/security/business/microsoft-intune",
      blurb: "Device compliance, patching & policies for Windows/macOS/iOS.",
    },
    {
...
// -------------------------------
const QUESTIONS = [
  { key: "employees", label: "How many employees do you have?", type: "select", options: ["1", "2-5", "6-15", "16-50", "51-200", "200+"] },
  { key: "suite", label: "Primary work suite?", type: "select", options: ["Microsoft 365", "Google Workspace", "Both/Other"] },
  { key: "mfa", label: "Is MFA enforced for all accounts?", type: "select", options: ["All users", "Some users", "Not enforced"] },
  { key: "pwdmgr", label: "Team password manager in use?", type: "select", options: ["Yes", "No"] },
  { key: "endpoint", label: "Endpoint protection (EDR/AV) on all devices?", type: "select", options: ["All devices", "Some devices", "No"] },
  { key: "backup", label: "Automated, offsite backups for important data?", type: "select", options: ["Yes", "No"] },
  { key: "emailsec", label: "Extra email security beyond built-in?", type: "select", options: ["Yes", "No"] },
  { key: "remote", label: "How many remote workers?", type: "select", options: ["None", "Some", "Many/Most"] },
  { key: "mdm", label: "Use MDM/device management (Intune/Jamf/etc.)?", type: "select", options: ["Yes", "Partial", "No"] },
  { key: "pii", label: "Process payments or store customer PII?", type: "select", options: ["Yes", "No"] },
] as const;
    ...
    steps.push({
      ...
      how: [
        a.suite === "Microsoft 365"
          ...
          : "Use Duo to enforce MFA across heterogeneous apps."
      ],
      category: "mfa",
      impact: 5,
      effort: "Low",
...
export default function SecurityAuditMVP() {
  ...
  function handleStart() {
    setStarted(true);
    (window as any)?.dataLayer?.push({ event: "start_audit" });
  }
        ...
        <Card className="p-8">
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div>
              ...
              <p className="mt-3 text-gray-700">Get a prioritized 10-minute plan to reduce risk: MFA, passwords, endpoint, backups & email security.</p>
              <div className="mt-6 flex items-center gap-3">
                {!started ? (
                  <button onClick={handleStart} className="inline-flex items-center justify-center rounded-xl bg-blue-600 px-5 py-3 text-white font-medium shadow hover:bg-blue-700">
                    Start Audit
                  </button>
                ) : (
                ...
                <a href="#how-it-works" className="inline-flex items-center justify-center rounded-xl border border-gray-300 px-5 py-3 font-medium hover:bg-gray-50">
                  How it works
                </a>
              </div>
      ...
      <section id="audit" className="max-w-5xl mx-auto px-4 py-8">
        <Card className="p-6">
          ...
          <div className="mt-6 flex items-center justify-between">
            ...
            <button
              disabled={!complete}
              onClick={() => { setShowPlan(true); if (complete) (window as any)?.dataLayer?.push({ event: "view_plan" }); }}
              className={`rounded-xl px-5 py-3 font-medium shadow ${complete ? "bg-blue-600 text-white hover:bg-blue-700" : "bg-gray-200 text-gray-500 cursor-not-allowed"}`}
            >
              Get My Plan
            </button>
          </div>
                          ...
                          <div className="mt-2 grid md:grid-cols-2 gap-3">
                            {(AFFILIATE[s.category] || []).map(opt => (
                              <a
                                ...
                                onClick={() => (window as any)?.dataLayer?.push({ event: "affiliate_click", category: s.category, vendor: opt.name })}
      ...
      <section id="how-it-works" className="max-w-5xl mx-auto px-4 py-8">
        <Card className="p-6">
          ...
          <ol className="list-decimal pl-6 space-y-2 text-sm text-gray-700">
            ...
            <li>Copy your plan and execute step-by-step.</li>
          </ol>
        </Card>
      </section>
